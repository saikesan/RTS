...
/*
 * Чтобы изменить имя файла, перейди на 329 строку и замени ИМЯ_ФАЙЛА на имя твоего файла,
...
int main()
{
	...
	//
	// file - описатель объекта асинхронной операции, в данном случае файл.
	// readingBuffer - указатель в который будет записан начальный адрес считанного блока памяти.
	// blockSize - сколько мы хотим считать.
	// NULL - параметр, в который, при синхронном выполении будет записано сколько мы реально считали байт.
	// &ol - указатель на структуру OVERLAPPED. Как раз из-за того, 
	//		 что мы его передали, операция чтения будет проходить асинхронно.
	...
	// Получаем результат асинхронного чтения. В currentRead 
	// будет занесено то, сколько байт реально было считано.
	// 
	// file - описатель объекта асинхронной операции, в данном случае файл.
	// &ol - указатель на структуру OVERLAPPED, который мы передавали при вызове функции ReadFile.
	// &currentRead - переменная в которую будет записано, сколько мы реально считали байт.
	// TRUE - параметр, отвечающий за то должны ли мы встать на паузу во 
	//		  время выполнения этой функции до тех пор пока асинхронная 
	//		  операция не будет закончена. Если TRUE - то да, будем ждать.
